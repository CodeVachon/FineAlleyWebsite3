component extends="object" displayname="utilities" hint="I am a group of utilities" {
	public utilities function init() { return super.init(); }
...
// *******************************************************************************************************
	...
	public string function arrayToJson(required array toSerialize) {
		var _array = [];
		for (var _arrayItem in ARGUMENTS.toSerialize) {
			if (isObject(_arrayItem)) {
				arrayAppend(_array,_arrayItem.getPropertyStruct());
			} else {
				arrayAppend(_array,_arrayItem);
			}
		}
		return serializeJson(_array);
	} // close arrayToJson

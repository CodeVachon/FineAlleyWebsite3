component displayname="object" hint="this is the base object that all objects should extend from" {
	...
	public boolean function hasMethod(required string methodName) {
		return structKeyExists(VARIABLES,ARGUMENTS.methodName);
	}

	public any function getMethod(required string methodName,string args) {
		if (!this.hasMethod(ARGUMENTS.methodName)) { return false; }
		var theMethod = VARIABLES[ARGUMENTS.methodName];
		if (!structKeyExists(ARGUMENTS,'args')) { ARGUMENTS.args = false; }
		return theMethod(ARGUMENTS.args);
	}
}
component {
	...
	public boolean function addMetaTag() {
		...
		if (structKeyExists(ARGUMENTS,"href")) { metaData["href"] = trim(ARGUMENTS.href); }
		return arrayAppend(VARIABLES.metaTags,metaData);
	}

	public boolean function addLinkTag() {
		if (!structKeyExists(ARGUMENTS,"href")) { throw("link requires an href attribute."); }
		var linkData = {};
		for (var key in ARGUMENTS) {
			linkData[key] = ARGUMENTS[key];
		}
		return arrayAppend(VARIABLES.linkTags,linkData);
	...
	public boolean function addFile(required string path) {
		if (len(ARGUMENTS.path) == 0) { throw("invalid path"); }
			...
			for (var i=1;i<=listLen(ARGUMENTS.path);i++) {
				...
				if (thisPath == "http://fonts.googleapis.com/css?family=Source+Sans+Pro:400&##44;900|Gudea:400&##44;700") {
					thisVar = "cssFiles";
	...
	public boolean function removeFile(required string path) {
		if (len(ARGUMENTS.path) == 0) { throw("invalid path"); }
		switch(reReplaceNoCase(ARGUMENTS.path,".+(\.\w{2,3})","\1","one")) {
			case ".png":
			case ".ico": thisVar="icoFiles"; break;
			case ".css": thisVar="cssFiles"; break;
			default: thisVar="jsFiles"; break;
		...
		return ArrayDelete(VARIABLES[thisVar],ARGUMENTS.path);
	}
